					...
					<c:forEach items="${allProduct}" var="p">
						...
						<div>
							<div class="product-item">
								...
								<c:forEach items="${newestProduct}" var="sp">
								<c:if test="${sp.pId==p.pId}">
...
<div class="col-md-12">
		<ul class="pagination">
		 <c:if test="${tag>1}">
			<li><a href="product?bid=${tagactive}&index=${tag-1}">Trước</a></li>
			</c:if>
			<c:forEach begin="1" end="${endP}" var="i">
			<li class="${tag==i ? "active":"" }"><a href="product?bid=${tagactive}&index=${i}">${i}</a></li>
			 </c:forEach>
			 <c:if test="${tag<endP}">
			<li><a href="product?bid=${tagactive}&index=${tag+1}">Tiếp</a></li>
			 </c:if>		
		</ul>
			 </div>
		<!-- END PAGING -->
		...
		<div class="row margin-bottom-40 ">
			...
			<div class="sidebar col-md-3 col-sm-4">
				<ul class="list-group margin-bottom-25 sidebar-menu">
					<c:forEach items="${allBrand}" var="b">
						<li class="list-group-item clearfix ${tagactive==b.bId ? "active" : ""}">
							<a href="product?bid=${b.bId}"><i class="fa fa-angle-right"></i>
								${b.bName }</a>
						</li>
					</c:forEach>
				</ul>
			...
			<div class="col-md-9 col-sm-8">
				...
				<c:forEach items="${sallestProduct}" var="p">
				<div>
						<div class="product-item">
							<div class="pi-img-wrapper">
								<img src="${ p.pImage }"
									class="img-responsive" alt="${ p.pName }">
							</div>
							<h3>
								<a href="productdetail?pid=${p.pId }">${ p.pName }</a>
							</h3>
